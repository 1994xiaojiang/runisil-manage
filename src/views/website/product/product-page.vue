    ...
    <div>
        <div v-if="secondpage" class="className">
            <el-card class="anoCard">
                <div slot="header">
                    <span style="font-weight: 600;font-size: 20px;">产品列表</span>
                </div>
                <div class="searchDiv">
                    <el-input type="text" placeholder="请输入产品名称" class="width1" v-model="sch_order"></el-input>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 下架
            toDelete(row) {
                updateStatusProduct({id:row.id,status:2}).then( res=>{
                    if(res.status === 'SUCCESS'){
                        this._getPageTab2();
                        this.$notify({
                            title: '成功',
